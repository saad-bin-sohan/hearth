...
void main() {
  ...
  testWidgets(
    'Splash redirects to sign in when onboarding is complete but no session exists',
  ...
  testWidgets(
    'Authenticated household session exposes Finance and Members in More',
    (WidgetTester tester) async {
      SharedPreferences.setMockInitialValues(<String, Object>{
        'onboarding_complete': true,
        'auth_token': 'token-1',
  ...
  testWidgets(
    'Sign up flows into create household and lands on invite screen',
    (WidgetTester tester) async {
      SharedPreferences.setMockInitialValues(<String, Object>{});
      final preferences = await SharedPreferences.getInstance();
      final database = createTestDatabase();
      ...
      await tester.ensureVisible(
        find.widgetWithText(HearthButton, 'Create Account'),
      );
      ...
      await tester.enterText(find.byType(TextFormField).last, 'Hearth House');
      await tester.ensureVisible(
        find.widgetWithText(HearthButton, 'Create Household'),
      );
